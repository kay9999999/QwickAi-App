          ...
          <div className="bg-[#F7F7FC] rounded-lg shadow p-3 sm:p-4 h-[317px] flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 space-y-2 sm:space-y-0">
              <div className="flex space-x-4 sm:space-x-8">
                {['Total Users ', 'Total Conversion', 'Intent Status'].map((text, idx) => (
                  <button key={idx} className={`text-xs sm:text-sm ${idx === 0 ? 'font-inter text-[#19213D] font-medium' : 'text-black/40'}`}>
                    {text}
                  </button>
                ))}
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <button className="text-xs sm:text-sm font-inter font-normal leading-4 tracking-normal text-[#19213D]">This Week</button>
                <span className="w-1 h-1 bg-gray-400 rounded-full"></span>
                <button className="text-xs sm:text-sm font-inter font-normal leading-4 tracking-normal text-[#19213D]">Last Week</button>
              </div>
            </div>
            <div className="flex-1">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={lineChartData} margin={{ top: 10, right: 0, left: 10, bottom: 0 }}>
                  <XAxis
                    dataKey="name"
                    axisLine={false}
                    tickLine={false}
                  ...
                  />
                  <Tooltip />
                  <Line type="monotone" dataKey="value1" stroke="#5e72e4" strokeWidth={2} dot={false} activeDot={{ r: 6 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="lg:w-1/2">
          <div className="bg-[#F7F7FC] rounded-lg shadow p-3 sm:p-4 h-[317px] flex flex-col overflow-hidden">
            <div className="flex justify-between items-center mb-1">
              <h3 className="text-base sm:text-lg font-poppins font-medium text-[#1C1C1C] leading-tight mt-2">Header</h3>
            ...
            <div className="flex-1 overflow-hidden">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={lineChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="1 1" vertical={false} stroke="#f0f0f0" strokeOpacity={0.4} />
                  <YAxis
                    axisLine={false}
                    tickLine={false}
                    domain={[0, 40000]}
                  ...
                  <Tooltip />
                  <Line type="monotone" dataKey="value1" stroke="#5e72e4" strokeWidth={2} dot={false} activeDot={{ r: 6 }} />
                  <Line type="monotone" dataKey="value2" stroke="#2dce89" strokeWidth={1.5} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <div className="flex justify-between">
              ...
              <div className="flex flex-col text-right">
                <span className="text-[#585757] font-poppins text-sm font-medium tracking-normal leading-[110%]">98 123</span>
                <span className="text-[#585757] font-poppins text-sm font-medium tracking-normal leading-[110%]">98 753</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mt-6">
        ...
        <div className="bg-[#F7F7FC] rounded-lg shadow p-3 sm:p-4">
          <div className="flex justify-between items-start mb-4">
            <div className="flex flex-col">
              <h3 className="text-sm sm:text-lg font-poppins font-medium text-[#949596]">Label</h3>
              <p className="text-sm sm:text-3xl font-poppins font-medium tracking-normal text-[#1C1C1C]">14,744</p>
            </div>
            <div className="flex flex-col items-end">
              ...
              <div className="flex items-center mt-4">
                ...
                <span className="text-xs sm:text-sm font-medium text-[#585757]">Legend</span>
              </div>
            </div>
          </div>
          ...
          <div className="h-[230px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barChartData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                ...
                <Tooltip />
                <Bar dataKey="value" fill="#5e72e4" barSize={8} radius={[4, 4, 0, 0]}>
                  {barChartData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={index % 3 === 0 ? '#625DCD' : '#4B8BF5'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <div className="h-[12px] flex mt-2">
            ...
            <div
              className="w-1/2"
              style={{
                background: 'linear-gradient(0deg, #4285F4, #4285F4), linear-gradient(0deg, rgba(255, 255, 255, 0.05), rgba(255, 255, 255, 0.05))',
              }}
            ></div>
          </div>
        ...
        <div className="bg-[#F7F7FC] rounded-lg shadow p-3 sm:p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-[16px] font-poppins font-medium tracking-normal leading-[110%] text-[#1C1C1C]">Header</h3>
          ...
          <div className="flex justify-center">
            <div className="relative w-full max-w-xs sm:max-w-md mx-auto h-72">
              ...
              <div className="absolute inset-0 flex items-center justify-center flex-col">
                <p className="text-sm sm:text-base font-medium text-gray-800">user</p>
                <span className="text-xl sm:text-3xl font-bold text-gray-800">32,455</span>
              </div>
            </div>
          </div>
